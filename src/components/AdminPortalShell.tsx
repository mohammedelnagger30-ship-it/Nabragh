...
import { ShieldCheck, GraduationCap, LogOut, Home, Moon, Sun } from 'lucide-react';
        ...
        <div className="mx-auto max-w-[1800px] px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            <Link to={isTeacher ? "/admin/teacher" : "/admin"} className="flex items-center gap-3 group">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {isTeacher ? 'إدارة محتواك وطلابك' : 'لوحة الإدارة'}
                </p>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className={`hidden items-center gap-2 rounded-xl px-3 py-1.5 border sm:flex ${
                isTeacher
                ...
                <div className={`h-1.5 w-1.5 rounded-full animate-pulse ${isTeacher ? 'bg-emerald-500' : 'bg-blue-500'}`} />
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">
                  {profile?.full_name || user?.user_metadata?.full_name || 'المستخدم'}
                </span>
              </div>
              <button onClick={signOut} className="flex items-center gap-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
