...
export default function CoursesPage() {
  ...
  const selectedLevel = advFilters.level?.[0] ?? null;
  ...
  useEffect(() => {
    ...
    setAdvFilters((current) => {
      ...
      if (profile.education_stage) merged.stage = [profile.education_stage];
      if (profile.curriculum) merged.curriculum = [profile.curriculum];
    ...
    <div className="pt-[4.5rem] min-h-screen bg-gradient-to-br from-slate-50 to-white dark:from-slate-950 dark:to-slate-900">
      ...
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-10 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-sm leading-6 text-slate-500 sm:text-base dark:text-slate-400">تصفح جميع الدورات المتاحة على المنصة</p>
        </div>

        {/* Search & Filters */}
        <div className="mb-6 rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-sm sm:p-5 dark:border-slate-700 dark:bg-slate-800">
          <div className="relative mb-4">
            ...
            <input
              ...
              className="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-4 pr-11 text-sm text-slate-800 placeholder:text-slate-400 transition-colors focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10 dark:border-slate-600 dark:bg-slate-900 dark:text-white"
            />
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${!selectedCategory ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600'}`}
            >
              كل التخصصات
            </button>
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${selectedCategory === cat.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600'}`}
              >
                {cat.name_ar}
              </button>
            ))}
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white rounded-2xl border border-slate-200 overflow-hidden animate-pulse dark:bg-slate-800 dark:border-slate-700">
          ...
          <>
            <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">{filtered.length} دورة {selectedStage && `في ${getEducationStageLabel(selectedStage)}`} {selectedCurriculum && `- ${getCurriculumLabel(selectedCurriculum)}`}</p>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
              {filtered.map((course) => (
                <Link
                  key={course.id}
                  to={`/course/${course.id}`}
                  className="group bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-lg hover:border-blue-200 transition-all hover:-translate-y-1 dark:bg-slate-800 dark:border-slate-700 dark:hover:border-blue-500"
                >
