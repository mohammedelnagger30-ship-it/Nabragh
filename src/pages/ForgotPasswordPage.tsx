...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 pb-10 pt-[6.5rem]">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60 sm:p-8">
        ...
        <div className="mb-7 text-center">
          ...
          <h1 className="text-2xl font-extrabold text-slate-900">استعادة كلمة المرور</h1>
          <p className="mt-2 text-sm leading-6 text-slate-500">أدخل بريدك وسنرسل لك رابطًا لإنشاء كلمة مرور جديدة.</p>
        </div>
        {sent ? (
          ...
          <form onSubmit={submit} className="space-y-5">
            <label className="block text-sm font-semibold text-slate-700">
              ...
              <div className="relative mt-2">
                ...
                <input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} dir="ltr" placeholder="you@example.com" className="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-4 pr-11 text-sm font-normal text-slate-800 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" />
              </div>
            </label>
